...
export const PageBackButton: React.FC<PageBackButtonProps> = ({
  ...
  return (
    <button
      type="button"
      onClick={goBack}
      className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white dark:bg-slate-800 border border-slate-200/90 dark:border-slate-700/90 text-xs font-bold text-slate-700 dark:text-slate-200 hover:text-teal-600 dark:hover:text-teal-400 hover:border-teal-500/50 shadow-2xs hover:shadow-xs transition active:scale-95 group ${className}`}
      aria-label={`Go back to ${targetLabel}`}
      title={`Go back to ${targetLabel}`}
    >
      <ArrowLeft className="w-3.5 h-3.5 text-slate-400 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-transform group-hover:-translate-x-1" />
      <span>Back to {targetLabel}</span>
    </button>
  );
};
